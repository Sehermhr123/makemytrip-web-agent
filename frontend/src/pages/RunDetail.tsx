    ...
    <div className="mx-auto max-w-5xl space-y-5 px-8 py-8">
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-border/70 pb-6">
        <div>
          ...
          <p className="mt-1.5 text-xs text-muted-foreground">Task {run.task_id.slice(0, 8)}</p>
        </div>
          ...
          {run.state === "AWAITING_APPROVAL" && (
            <>
              ...
              <Button
                variant="outline"
                className="h-9 px-3"
                onClick={() => reject.mutate({ runId: run.id })}
              ...
              <tbody>
                {(steps ?? []).map((s, i) => (
                  <tr key={i} className="border-b border-border last:border-0 hover:bg-secondary/30">
                    <td className="px-5 py-3 font-mono text-xs text-muted-foreground">{s.previous_state ?? "-"}</td>
                    <td className="px-2 py-3 text-muted-foreground">to</td>
                    <td className="px-2 py-3 font-mono text-xs font-medium">{s.new_state}</td>
      ...
      {(evidenceRows ?? []).length > 0 && (
        <Card>
          ...
          <CardContent className="space-y-3 px-5 py-4">
            {(evidenceRows ?? []).map((e) => (
              <div key={e.id} className="flex items-center gap-4 border-b border-border pb-3 text-sm last:border-0 last:pb-0">
                {e.artifact_purged ? (
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-0.5 text-xs text-muted-foreground">{new Date(e.captured_at).toLocaleString()}</div>
                </div>
              </div>
            ))}
          </CardContent>
          ...
          <CardContent className="p-0">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <tbody>
                  {evidence.map((snap) => (
                    <tr key={snap.id} className="border-b border-border last:border-0 hover:bg-secondary/30">
                      <td className="px-5 py-3 font-medium">{snap.entity_key}</td>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <tbody>
                  {changes.map((c, i) => (
                    <tr key={i} className="border-b border-border last:border-0 hover:bg-secondary/30">
                      <td className="px-5 py-3 font-medium">{c.entity_name}</td>
                      <td className="px-2 py-3 font-mono text-xs">{c.change_type}</td>
                      <td className="px-2 py-3 text-xs text-muted-foreground">{c.previous_value ?? "-"} to {c.current_value ?? "-"}</td>
                      <td className="px-2 py-3 text-xs">{c.delta_pct != null ? `${c.delta_pct}%` : ""}</td>
        ...
        <Card>
          ...
          <CardContent className="space-y-4 px-5 py-4">
            {signals.map((s) => {
              ...
              return (
                <div key={s.id} className="border-b border-border pb-4 text-sm last:border-0 last:pb-0">
                  ...
                  <p className="mt-1.5 leading-6 text-muted-foreground">{s.business_impact ?? s.observations}</p>
                  {groundingCall && (
                ...
                <tbody>
                  {(modelCalls ?? []).map((c) => (
                    <tr key={c.id} className="border-b border-border last:border-0 hover:bg-secondary/30">
                      ...
                      <td className="px-2 py-3 text-xs text-muted-foreground">{c.model_name}</td>
                      <td className="px-2 py-3 text-xs text-muted-foreground">{c.purpose}</td>
                      <td className="px-2 py-3 text-xs">{c.latency_ms}ms</td>
                      <td className="px-5 py-3 text-right">
                        {c.success ? (
