...
function StatLink({ label, value, to }: { label: string; value: number | string; to: string }) {
  return (
    <Link to={to}>
      <Card className="border-border bg-background shadow-sm transition-colors hover:border-primary/50">
        <CardContent className="py-4">
          <div className="text-2xl font-bold tracking-tight">{value}</div>
          <div className="mt-1 text-xs text-muted-foreground">{label}</div>
        </CardContent>
  ...
  return (
    <Card className="border-border bg-background shadow-sm">
      <CardHeader className="pb-3">
        <CardTitle className="text-base font-semibold">Runs per day (last 14 days)</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="h-56">
...
function SignalsBySeverityChart({ data }: { data: ReturnType<typeof signalsBySeverity> }) {
  ...
  return (
    <Card className="border-border bg-background shadow-sm">
      <CardHeader className="pb-3">
        <CardTitle className="text-base font-semibold">Signals by severity</CardTitle>
      </CardHeader>
      <CardContent>
        {total === 0 ? (
...
function WorkflowBreakdownChart({ data }: { data: ReturnType<typeof workflowBreakdown> }) {
  return (
    <Card className="border-border bg-background shadow-sm">
      <CardHeader className="pb-3">
        <CardTitle className="text-base font-semibold">Workflow breakdown</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="h-56">
          <ResponsiveContainer width="100%" height="100%">
...
function AiActivityCard({ summary }: { summary: ReturnType<typeof summarizeAiActivity> }) {
  return (
    <Card className="border-border bg-background shadow-sm">
      <CardHeader className="pb-3">
        <CardTitle className="text-base font-semibold">AI activity today</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <div className="text-2xl font-bold tracking-tight">{summary.totalCalls}</div>
            <div className="mt-1 text-xs text-muted-foreground">Calls made</div>
          </div>
          <div>
...
function MonitoredDestinationsMap({
  ...
  if (destinations.length === 0) {
    return (
      <Card className="border-border bg-background shadow-sm">
        <CardHeader className="pb-3">
          <CardTitle className="text-base font-semibold">Monitored destinations</CardTitle>
        </CardHeader>
  ...
  return (
    <Card className="border-border bg-background shadow-sm">
      ...
      <CardContent>
        <div className="h-72 overflow-hidden rounded-md border border-border/70">
        ...
        <div className="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-xs text-muted-foreground">
          <span className="flex items-center gap-1.5">
            <span className="h-2 w-2 rounded-full" style={{ background: PIN_COLORS.none }} />
            No open signals
          </span>
          <span className="flex items-center gap-1.5">
            <span className="h-2 w-2 rounded-full" style={{ background: PIN_COLORS.notable }} />
            Notable
          </span>
          <span className="flex items-center gap-1.5">
            <span className="h-2 w-2 rounded-full" style={{ background: PIN_COLORS.significant }} />
            Significant
          </span>
        </div>
      </CardContent>
            ...
            <tbody>
              {(runs ?? []).slice(0, 8).map((r) => (
                <tr key={r.id} className="border-b border-border last:border-0">
                  ...
                  <td className="px-2 py-3 text-xs text-muted-foreground">{r.state}</td>
